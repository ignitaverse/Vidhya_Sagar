...
app.get('/quiz/states', async (req, res) => {
  try {
    ...
    const result = await turso.execute({
      sql:  `SELECT question, options, answer, description FROM "states_quiz" WHERE category = ? ORDER BY RANDOM() LIMIT 10`,
      args: [state]
    });
...
app.get('/quiz/:subject', async (req, res) => {
  try {
    ...
    let sql  = `SELECT question, options, answer, description FROM "${table}"`;
    const args = [];
