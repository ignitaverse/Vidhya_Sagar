...
// ─── MIDDLEWARE ───
app.use(cors({
  origin: ['https://ignitaverse.github.io','http://localhost:5500','http://127.0.0.1:5500','https://vidhya-sagar.onrender.com'],
  credentials: true
}));
...
// QUIZ ROUTES (Turso)
// ══════════════════════════════════════════
// States — must come BEFORE /:subject
app.get('/quiz/states', async (req,res) => {
  try {
    ...
    if (excludeIds.length) { w += ` AND rowid NOT IN (${excludeIds.map(()=>'?').join(',')})`; excludeIds.forEach(id=>args.push(id)); }
    const total = Number((await turso.execute({ sql:`SELECT COUNT(*) as cnt FROM "states_quiz" ${w}`, args:[...args] })).rows[0]?.cnt ?? 0);
    if (!total) return res.json({ success:true, data:[], remaining:0, exhausted:true });
    const result = await turso.execute({ sql:`SELECT rowid,question,options,answer,description FROM "states_quiz" ${w} ORDER BY RANDOM() LIMIT ${batchSize}`, args });
    const questions = result.rows.map(r => ({ id:r.rowid, q:r.question, opts:typeof r.options==='string'?JSON.parse(r.options):r.options, ans:Number(r.answer), description:r.description||'' }));
...
app.get('/quiz/states/categories', async (req,res) => {
  try {
    const r = await turso.execute({ sql:`SELECT DISTINCT category FROM "states_quiz" WHERE category IS NOT NULL AND category!='' ORDER BY category`, args:[] });
...
app.get('/quiz/:subject/categories', async (req,res) => {
  try {
    ...
    const r = await turso.execute({ sql:`SELECT DISTINCT category FROM "${table}" WHERE category IS NOT NULL AND category!='' ORDER BY category`, args:[] });
    res.json({ success:true, categories:r.rows.map(x=>x.category) });
  } catch(e) { res.status(500).json({ success:false, message:e.message }); }
});
...
app.get('/quiz/:subject', async (req,res) => {
  try {
    ...
    if (excludeIds.length) { where.push(`rowid NOT IN (${excludeIds.map(()=>'?').join(',')})`); excludeIds.forEach(id=>args.push(id)); }
    const w = where.length ? ` WHERE ${where.join(' AND ')}` : '';
    const total = Number((await turso.execute({ sql:`SELECT COUNT(*) as cnt FROM "${table}"${w}`, args:[...args] })).rows[0]?.cnt ?? 0);
    if (!total) return res.json({ success:true, data:[], remaining:0, exhausted:true });
    const result = await turso.execute({ sql:`SELECT rowid,question,options,answer,description FROM "${table}"${w} ORDER BY RANDOM() LIMIT ${batchSize}`, args });
    const questions = result.rows.map(r => ({ id:r.rowid, q:r.question, opts:typeof r.options==='string'?JSON.parse(r.options):r.options, ans:Number(r.answer), description:r.description||'' }));
    res.json({ success:true, data:questions, remaining:total-questions.length });
  } catch(e) { res.status(500).json({ success:false, message:e.message }); }
});
...
app.get('/api/stats', async (req,res) => {
  try {
    const User = require('./models/User'); const QH = require('./models/QuizHistory');
    const fiveAgo = Date.now() - 5*60*1000;
    for (const [id,ts] of guestPings) { if (ts < fiveAgo) guestPings.delete(id); }
    res.json({ success:true, totalUsers:await User.countDocuments(), liveGuests:guestPings.size, totalQuizzes:await QH.countDocuments() });
  } catch(e) { res.status(500).json({ success:false, message:e.message }); }
});
...
app.post('/api/ai/chat', async (req,res) => {
  try {
    ...
    const response = await anthropic.messages.create({
      ...
      max_tokens: 600,
      system: 'You are VidyaSagar AI Study Buddy — a helpful assistant for Indian Government Exam students. Answer in Hinglish (mix of Hindi and English). Keep answers concise and practical. Focus on exam-related topics: GK, Maths, Reasoning, English, Hindi, Computer, Current Affairs, and typing practice.',
...
// ══════════════════════════════════════════
// GROUP CHAT (MongoDB fallback — primary is Firebase)
// ══════════════════════════════════════════
...
app.post('/api/chat', async (req,res) => {
  try {
    ...
    const ChatMessage = require('./models/ChatMessage');
    const msg = await ChatMessage.create({ user:user._id, userName:user.name, userAvatar:user.avatar||'🎓', message:message.trim() });
...
app.put('/api/notifications/read', async (req,res) => {
  res.json({ success:true }); // track in client-side localStorage
});

// Admin: create notification (protected by ADMIN_SECRET env var)
...
app.get('/api/feedback', async (req,res) => {
  try { const FB=require('./models/Feedback'); res.json({ success:true, feedback:await FB.find().sort({postedAt:-1}).limit(20) }); }
  catch(e) { res.status(500).json({ success:false, message:e.message }); }
});
...
mongoose.connect(process.env.MONGODB_URI)
  .then(() => {
    ...
    // Auto-delete inactive users (3+ months)
    async function cleanInactiveUsers() {
      try {
        ...
        for (const u of inactive) {
          await Promise.all([QH.deleteMany({user:u._id}),TH.deleteMany({user:u._id}),CM.deleteMany({user:u._id}),FB.deleteMany({userId:u._id}),User.findByIdAndDelete(u._id)]);
        }
        if (inactive.length) console.log(`[AutoClean] Removed ${inactive.length} inactive users`);
...
app.get('/', (req,res) => res.json({
  status:'✅ VidyaSagar v3',
  mongodb: mongoose.connection.readyState===1 ? 'Connected' : 'Disconnected',
  routes: { quiz:'/quiz/:subject', states:'/quiz/states', ai:'/api/ai/chat', chat:'/api/chat', notif:'/api/notifications', auth:'/api/auth', history:'/api/history', typing:'/api/typing' }
}));
app.use((req,res) => res.status(404).json({ success:false, message:'Not found' }));
app.use((err,req,res,next) => { console.error(err); res.status(500).json({ success:false, message:'Server error' }); });

const PORT = process.env.PORT || 5000;
app.listen(PORT, () => console.log(`🚀 VidyaSagar v3 on port ${PORT}`));
