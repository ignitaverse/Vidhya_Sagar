...
async function openCategoryScreen(sub){
  ...
  // Check for saved progress and show resume card
  checkResumeCard();
...
/* ══════════════════════════════════════════════════════════
   ...
   ─ Key: vs_prog_<subjectId>_<category>
   ─ Value: { seenIds, score, wrong, answered, total, elapsed }
══════════════════════════════════════════════════════════ */
function progKey(){
...
/* ── Quiz state (runs across batches) ── */
let quizState = {
  seenIds:  [],   // all rowids answered so far
  score:    0,    // total correct
  wrong:    0,    // total wrong
  answered: 0,    // total questions answered
  total:    0,    // total questions in this category
  isLast:   false // true when server says remaining=0
};

/* ── Fetch one batch from server ── */
async function fetchBatch(){
  const excl = quizState.seenIds.length > 0
    ? `&exclude=${quizState.seenIds.join(',')}` : '';
  let d;
  if(currentState){
    d = await apiFetch(`/quiz/states?state=${encodeURIComponent(currentState)}${excl}`);
  } else {
...
async function startQuiz(resuming=false){
  ...
  if(resuming){
    showToast(`Resuming from Q${quizState.answered+1} 📖`, 'info');
  }
...
async function loadNextBatch(){
  try{
    ...
    if(d.exhausted || !d.data || d.data.length===0){
      // All questions done — show final result
      clearProgress();
      showFinalResult();
      return;
    }
    ...
    // Total questions = seen + remaining + current batch
...
function renderQuestion(){
  if(currentQ >= questions.length){
    // Batch done — load next batch or show result
    if(quizState.isLast){
      clearProgress();
      showFinalResult();
    } else {
...
function startTimer(){
  ...
  updateTimer();
  timerInterval = setInterval(()=>{ elapsedSeconds++; updateTimer(); saveProgress(); },1000);
}
...
function submitQuizNow(){
  // Save current batch progress first
  const q = questions[currentQ];
  if(q && !answered){
    // Count unanswered as wrong? No — just stop here
  }
  stopTimer();
  saveProgress();
  showFinalResult();
...
function showFinalResult(){
  ...
  // Check if fully complete — hide resume button
...
function bindEvents(){
  ...
  document.getElementById('btn-explain').addEventListener('click',()=>{
    const q=questions[currentQ];
    document.getElementById('desc-text').textContent=q.description||'No explanation available.';
  ...
  // Continue (retry = load next batch)
  document.getElementById('btn-retry').addEventListener('click',()=>{
    const prog = loadProgress();
    if(prog && prog.answered > 0){
      // Continue from saved
      quizState = {
        seenIds:  prog.seenIds||[],
        score:    prog.score||0,
        wrong:    prog.wrong||0,
