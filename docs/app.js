...
══════════════════════════════════════════════════════ */
/* ── SEEN IDs TRACKER ─────────────────────────────────────────
   Key: vs_seen_<subjectId>_<category>
   Value: comma-separated rowids already answered
─────────────────────────────────────────────────────────── */
function seenKey(){
  const sub = currentState ? 'states' : currentSubject.id;
  const cat = currentState || currentCategory || 'all';
  return `vs_seen_${sub}_${cat}`;
}
function getSeenIds(){
  const val = localStorage.getItem(seenKey());
  if(!val) return [];
...
function addSeenIds(ids){
  ...
  localStorage.setItem(seenKey(), merged.join(','));
}
...
async function startQuiz(){
  ...
  try{
    ...
    let d;
    if(currentState){
      d=await apiFetch(`/quiz/states?state=${encodeURIComponent(currentState)}${excludeParam}`);
    }else{
    ...
    // If exhausted — reset seen and start fresh
    if(d.exhausted || !d.data || d.data.length===0){
      clearSeenIds();
      showToast('🎉 All questions done! Starting fresh from Q1','success');
    ...
    const totalSeen = getSeenIds().length;
    const remaining = d.remaining ?? '?';
    document.getElementById('score-left').textContent=questions.length;

    // Show progress pill if we have seen-count info
    if(totalSeen>0){
      showToast(`Continuing from Q${totalSeen+1} 📖`,'info');
    }
...
function renderQuestion(){
  ...
  // Reset explanation on every new question
...
function handleAnswer(idx){
  ...
  // Show explanation button only if description exists for this question
...
function endQuiz(){
  ...
  // ── Save seen IDs so next session continues from here ──
  const seenNow = questions.map(q=>q.id).filter(Boolean);
  if(seenNow.length>0) addSeenIds(seenNow);
  const totalDone = getTotalSeenCount();
  document.getElementById('seen-count-badge').textContent=`${totalDone} questions done so far`;
  document.getElementById('seen-count-badge').classList.remove('hidden');
