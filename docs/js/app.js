...
/* ══════════════════════════════════════
   NAVIGATION — Back button fix (History API)
══════════════════════════════════════ */
let _subStack = []; // open sub-screens ka stack

function switchTab(tab){
  /* Sabhi sub-screens band karo — YAHI tha chat bug */
  document.querySelectorAll('.sub-screen.open').forEach(s=>{
...
async function loadNotifications(){
  ...
  try{
    ...
    if(!items.length){list.innerHTML='<div class="vs-empty"><span class="ve-icon">🔔</span>No notifications yet</div>';return;}
    list.innerHTML='';
    items.forEach(n=>{
...
function initUserSearch(){
  ...
  inp.addEventListener('focus',()=>{
    document.getElementById('search-box-wrap').style.borderColor='var(--blue2)';
  });
  inp.addEventListener('blur',()=>{
    setTimeout(()=>{ drop.style.display='none'; },200);
    document.getElementById('search-box-wrap').style.borderColor='';
  });
}
...
async function doUserSearch(q){
  ...
  try{
    ...
    const users=d.users||[];
    if(!users.length){drop.innerHTML='<div style="padding:14px;text-align:center;color:var(--text3);font-size:.82rem">No users found</div>';return;}
    drop.innerHTML=users.map(u=>`
      <div class="sri-item" onclick="showToast('Public profile coming soon! 🚀','info')">
        <div class="sri-av">${u.avatar||'🎓'}</div>
        <div>
          <div class="sri-name">${_escH(u.name)}</div>
          <div class="sri-sub">${_escH(u.examPrep||'Student')}</div>
        </div>
      </div>`).join('');
...
function bindEvents(){
  ...
  ['tog-public','tog-online','tog-lastseen'].forEach(id=>{
    document.getElementById(id)?.addEventListener('change',async()=>{
      if(!token)return;
      try{await apiFetch('/api/auth/privacy',{method:'PUT',body:JSON.stringify({
