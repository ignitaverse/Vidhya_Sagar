...
const ProfileModule = (() => {
  ...
  function init() {
    ...
    // History links from profile stats row
  ...
  function render() {
    ...
    if (nm) nm.textContent = userData.name || 'User';
    const em = document.getElementById('ph-email');
    if (em) em.textContent = userData.email || '';
    const jn = document.getElementById('ph-joined');
    if (jn && userData.joinedAt) jn.textContent = 'Member since ' + new Date(userData.joinedAt).toLocaleDateString('en-IN',{month:'long',year:'numeric'});
    ...
    if (ain) { ain.value = userData.name||''; const left=2-(userData.nameChanges||0); ain.disabled=left<=0; const b=document.getElementById('name-changes-left'); if(b) b.textContent=left>0?`(${left} बार बदल सकते हैं)`:'(locked)'; }
    const aie = document.getElementById('ai-email'); if (aie) aie.value = userData.email||'';
  ...
  async function loadProfileStats() {
    try {
      ...
      const qList=qH.history||[]; const tList=tH.history||[];
  ...
  async function saveInfo() {
    ...
    try {
      ...
      const d=await apiFetch('/api/auth/profile',{method:'PUT',body:JSON.stringify({name:name||undefined,dob,examPrep,bio,avatar:selectedAvatar})});
  ...
  async function loadHistory() {
    ...
    try {
      const d=await apiFetch('/api/history');const h=d.history||[];
      ...
      h.forEach(x=>{
        ...
        card.innerHTML=`<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:8px"><span style="font-weight:700;font-size:.87rem">${subj}</span><div style="display:flex;align-items:center;gap:8px"><span style="background:rgba(0,0,0,.2);border-radius:6px;padding:3px 10px;font-weight:800;font-size:.84rem;color:${cls}">${x.percentage}%</span><button data-id="${x._id}" style="background:rgba(244,63,94,.1);border:1px solid rgba(244,63,94,.3);color:#f43f5e;border-radius:6px;padding:3px 8px;font-size:.72rem;cursor:pointer">🗑️</button></div></div><div style="height:4px;background:var(--card2);border-radius:4px;overflow:hidden;margin-bottom:8px"><div style="height:100%;width:${x.percentage}%;background:${cls};border-radius:4px"></div></div><div style="display:flex;gap:14px;font-size:.73rem;color:var(--text3)"><span>✅ ${x.score}/${x.total}</span><span>⏱ ${x.timeTaken}s</span><span>📅 ${date}</span></div>`;
        card.querySelector('[data-id]').addEventListener('click',async btn2=>{
          ...
          try{await apiFetch(`/api/history/${id}`,{method:'DELETE'});card.remove();showToast('Deleted ✅','success');}
  ...
  async function loadTypingHistory() {
    ...
    if(!token){list.innerHTML='<div class="vs-empty">Login करें</div>';return;}
    try {
      const d=await apiFetch('/api/typing/history');const h=d.history||[];
      ...
      h.forEach(x=>{
        ...
        const card=document.createElement('div');card.className='hist-card';
        card.innerHTML=`<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:8px"><span style="font-weight:700;font-size:.87rem">${x.examName}</span><span style="font-weight:800;font-size:.82rem;color:${pc}">${x.passed?'✅ PASS':'❌ FAIL'}</span></div><div style="display:grid;grid-template-columns:1fr 1fr 1fr 1fr;gap:7px;margin-bottom:6px"><div style="text-align:center"><div style="font-weight:800;font-size:1.1rem;color:#34d399">${x.wpm}</div><div style="font-size:.65rem;color:var(--text3)">WPM</div></div><div style="text-align:center"><div style="font-weight:800;font-size:1.1rem;color:var(--blue2)">${x.accuracy}%</div><div style="font-size:.65rem;color:var(--text3)">Accuracy</div></div><div style="text-align:center"><div style="font-weight:800;font-size:1.1rem;color:var(--rose)">${x.errors}</div><div style="font-size:.65rem;color:var(--text3)">Errors</div></div><div style="text-align:center"><div style="font-weight:800;font-size:1.1rem;color:var(--amber)">${x.keystrokes}</div><div style="font-size:.65rem;color:var(--text3)">Keys</div></div></div><div style="display:flex;gap:10px;font-size:.72rem;color:var(--text3)"><span>🌐 ${x.language}</span><span>⏱ ${x.timeTaken}s</span><span>📅 ${date}</span></div>`;
        list.appendChild(card);
      });
    }catch(e){list.innerHTML=`<div class="vs-empty">${e.message}</div>`;}
  }
  ...
  window.applyTheme=applyTheme;window.applyFont=applyFont;window.applyZoom=applyZoom;
  window.setChatBg=setChatBg;window.uploadChatBg=uploadChatBg;
  window.openSubScreen=openSubScreen;window.closeSubScreen=closeSubScreen;
