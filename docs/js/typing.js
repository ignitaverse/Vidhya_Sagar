/* ═══════════════════════════════════════════
   VidyaSagar v3 — typing.js REDESIGNED
   AR Typing style with search + grid cards
═══════════════════════════════════════════ */
const TypingModule = (() => {
  ...
  function init() {
    ...
    // Search input
    const searchInp = document.getElementById('exam-search-inp');
    if (searchInp) {
    ...
    list.forEach(exam => {
      ...
      card.innerHTML = `
        <div class="ecv2-header" style="background:linear-gradient(135deg,${exam.color}22,${exam.color}11)">
          ...
          ${badge}
          <div class="ecv2-langs">${langs}</div>
        </div>
        <div class="ecv2-body">
          <div class="ecv2-name">${exam.name}</div>
          <div class="ecv2-meta">
            <span>~${wordCount} words</span>
            <span>⏱ ${exam.time/60} min</span>
          </div>
  ...
  async function loadPassages(exam) {
    ...
    try {
      ...
      passages.forEach((p,i) => {
        ...
        const card = document.createElement('div');
        card.className = 'passage-card-v2';
        card.innerHTML = `
